...
function requireStringArray(
  ...
  if (
    ...
    throw new Wf08CliError(
      "WF08_INPUT_INVALID",
      `${label}.${key} must be ${allowEmpty ? "an" : "a non-empty"} array of non-empty strings.`
    );
  }
...
export function parseProjectStyleDecision(value: unknown): ProjectStyleDecision {
  if (!isRecord(value)) {
    throw new Wf08CliError(
      "WF08_INPUT_INVALID",
      "project-style.json must contain a JSON object."
    );
  }
  return {
    ...
    characterRenderingPrinciple: requireString(
      value,
      "characterRenderingPrinciple",
      "project-style.json"
    ),
    cameraCompositionTendency: requireString(
      value,
...
export function parseAnchorPlanDecision(value: unknown): AnchorPlanDecision {
  if (!isRecord(value) || !Array.isArray(value.anchors)) {
    throw new Wf08CliError(
      "WF08_INPUT_INVALID",
      "identity-anchors.json must contain an anchors array."
    );
  }
  ...
  return {
    anchors: value.anchors.map((candidate, index) => {
      ...
      if (!isRecord(candidate) || !isRecord(candidate.specification)) {
        throw new Wf08CliError(
          "WF08_INPUT_INVALID",
          `${label} must contain an object specification.`
        );
      }
      ...
      if (!ANCHOR_TYPES.has(anchorType as "CHARACTER" | "LOCATION" | "PROP")) {
        throw new Wf08CliError(
          "WF08_INPUT_INVALID",
          `${label}.anchorType must be CHARACTER, LOCATION or PROP.`
        );
      }
      if (!CONTINUITY_REASONS.has(
        continuityReason as "RECURRING" | "CRITICAL_CONTINUITY"
      )) {
        throw new Wf08CliError(
          "WF08_INPUT_INVALID",
          `${label}.continuityReason must be RECURRING or CRITICAL_CONTINUITY.`
        );
      }
      if (!PRIORITIES.has(
        productionPriority as "CRITICAL" | "IMPORTANT" | "SUPPORTING"
      )) {
        throw new Wf08CliError(
          "WF08_INPUT_INVALID",
          `${label}.productionPriority must be CRITICAL, IMPORTANT or SUPPORTING.`
        );
      }
      ...
      return {
        ...
        specification: {
          ...
          contextual: requireStringArray(
            candidate.specification,
            "contextual",
            `${label}.specification`
          ),
          temporary: requireStringArray(
            candidate.specification,
            "temporary",
            `${label}.specification`
          )
...
function unavailableDecisions(): VisualIdentityDecisionPort {
  return {
    async designProjectStyle() {
      throw new Wf08CliError(
        "WF08_USAGE",
        "Project Style decision input is unavailable for this command."
      );
    },
    async planIdentityAnchors() {
      throw new Wf08CliError(
        "WF08_USAGE",
        "Identity Anchor decision input is unavailable for this command."
      );
    }
  };
}
...
function visualBiblePin(status: ProjectStatus): ResourcePin {
  const pin = status.resourcePins.find(
    (candidate) => candidate.resourceType === "CHANNEL_VISUAL_BIBLE"
  );
  if (pin === undefined) {
    throw new Wf08CliError(
      "WF08_RESOURCE_PIN",
      "Project is missing its pinned Channel Visual Bible resource."
    );
  }
  return pin;
}
...
export class Wf08CliService {
  ...
  private async withRepository<T>(
    projectId: string,
    fn: (
      repo: SqliteVisualIdentityRepository,
  ...
  async applyProjectStyle(projectId: string, file: string) {
    return this.withRepository(projectId, async (repo, status) => {
      const decision = parseProjectStyleDecision(
        await readProjectJson(status.projectRoot, file)
      );
      const decisions: VisualIdentityDecisionPort = {
        ...
        async planIdentityAnchors() {
          throw new Wf08CliError(
            "WF08_USAGE",
            "Identity Anchor decision input is unavailable during Project Style apply."
          );
        }
      };
      const pipeline = new VisualIdentityPipeline(
        repo,
        repo,
        this.bible(status),
        decisions,
      ...
      return pipeline.generateProjectStyle({
        projectId,
        format: status.project.format,
        channelVisualBibleVersion: visualBiblePin(status).version
      });
    });
  }
  ...
  async approveProjectStyle(projectId: string, approvedById?: string) {
    return this.withRepository(projectId, async (repo, status) => {
      ...
      return pipeline.approveProjectStyle({
        projectId,
  ...
  async applyIdentityAnchors(projectId: string, file: string) {
    return this.withRepository(projectId, async (repo, status) => {
      const decision = parseAnchorPlanDecision(
        await readProjectJson(status.projectRoot, file)
      );
      const decisions: VisualIdentityDecisionPort = {
        async designProjectStyle() {
          throw new Wf08CliError(
            "WF08_USAGE",
            "Project Style decision input is unavailable during Identity Anchor apply."
          );
        },
        async planIdentityAnchors() {
          return decision;
      ...
      return pipeline.planIdentityAnchors({
        projectId,
        format: status.project.format
      });
    });
  ...
  async approveIdentityAnchors(
    projectId: string,
    anchorIds: string[] | "ALL",
    approvedById?: string
  ) {
    return this.withRepository(projectId, async (repo, status) => {
      const selected = anchorIds === "ALL"
        ? (await repo.listActiveAnchors(projectId)).map((anchor) => anchor.id)
        : [...new Set(anchorIds)];
      if (selected.length === 0) {
        throw new Wf08CliError(
          "WF08_USAGE",
          "No Identity Anchors were selected for approval."
        );
      }
      const pipeline = new VisualIdentityPipeline(
        repo,
      ...
      return pipeline.approveAnchors({
        projectId,
        anchorIds: selected,
        ...(approvedById === undefined ? {} : { approvedById })
      });
  ...
  async status(projectId: string) {
    return this.withRepository(projectId, async (repo, status) => {
      ...
      const projectStyleApproval = projectStyle === null
        ? null
        : await repo.getLatestApproval(projectId, "PROJECT_STYLE", projectStyle.id);
      const anchors = await Promise.all(identityAnchors.map(async (anchor) => ({
        ...anchor,
        approval: await repo.getLatestApproval(projectId, "IDENTITY_ANCHOR", anchor.id)
      })));
      const pipeline = new VisualIdentityPipeline(
        repo,
        repo,
      ...
      return {
        ...
        projectStyle: projectStyle === null
          ? null
          : { ...projectStyle, approval: projectStyleApproval },
        identityAnchors: anchors,
        readiness: await pipeline.getReadiness(projectId)
      };
    });
  }
}
