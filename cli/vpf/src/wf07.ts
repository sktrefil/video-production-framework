...
export class Wf07CliService {
  ...
  async addResearch(input: {
    ...
    return this.withRepository(input.projectId, async (repo) => {
      ...
      return pipeline.addResearchSource({
  ...
  async approveFact(projectId: string, factId: string) {
    return this.withRepository(projectId, async (repo) => {
      const pipeline = new StoryPipeline(repo, systemClock, createIds());
      return pipeline.approveFact(projectId, factId);
    });
  }
  ...
  async createScript(input: {
    ...
    return this.withRepository(input.projectId, async (repo, projectRoot) => {
      ...
      return pipeline.createScript({
        projectId: input.projectId,
        body,
        ...(normalizeScriptKind(input.kind) === undefined
          ? {}
          : { kind: normalizeScriptKind(input.kind)! })
      });
    });
  }
  ...
  async reviseScript(input: {
    ...
    return this.withRepository(input.projectId, async (repo, projectRoot) => {
      const body = await readProjectInput(projectRoot, input.file);
      const pipeline = new StoryPipeline(repo, systemClock, createIds());
      const kind = normalizeScriptKind(input.kind);
      return pipeline.reviseScript({
        projectId: input.projectId,
  ...
  async approveStructure(projectId: string, approvedById?: string) {
    return this.withRepository(projectId, async (repo) => {
      ...
      return service.approveStructure({
        projectId,
        ...(approvedById === undefined ? {} : { approvedById })
      });
    });
  ...
  async approveScenes(projectId: string, sceneIds: string[] | "ALL", approvedById?: string) {
    return this.withRepository(projectId, async (repo) => {
      ...
      if (selected.length === 0) {
        throw new Wf07CliError("WF07_USAGE", "No scenes were selected for approval.");
      }
      const service = new StoryGenerationService(
        repo,
        {
