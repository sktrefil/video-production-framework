...
async function readProjectJson(projectRoot: string, inputPath: string): Promise<unknown> {
  ...
  if (!isInside(resolvedRoot, resolvedInput)) {
    throw new Wf09bCliError("WF09B_INPUT_PATH", "WF-09B input files must remain inside the current project workspace.");
  }
  let actual: string;
  try {
    actual = await realpath(resolvedInput);
  } catch {
    throw new Wf09bCliError("WF09B_INPUT_PATH", `WF-09B input file does not exist or is unreadable: ${inputPath}`);
  }
  if (!isInside(resolvedRoot, actual)) {
    throw new Wf09bCliError("WF09B_INPUT_PATH", "WF-09B input symlink resolves outside the current project workspace.");
  }
  try {
    return JSON.parse(await readFile(actual, "utf8")) as unknown;
  } catch {
    throw new Wf09bCliError("WF09B_INPUT_INVALID", `WF-09B input must contain valid JSON: ${inputPath}`);
  }
}

function requireImageProviderPin(status: ProjectStatus): ResourcePin {
  const pin = status.resourcePins.find(candidate =>
    candidate.resourceType === "PROVIDER_PROFILE" && candidate.resourceId === IMAGE_PROVIDER_PROFILE_ID
  );
  if (pin === undefined) throw new Wf09bCliError("WF09B_RESOURCE_PIN", `Project is missing pinned ${IMAGE_PROVIDER_PROFILE_ID}.`);
  return pin;
}
...
function requirePin(status: ProjectStatus, resourceType: "CHANNEL_VISUAL_BIBLE" | "FORMAT_PROFILE"): ResourcePin {
  ...
  if (pin === undefined) throw new Wf09bCliError("WF09B_RESOURCE_PIN", `Project is missing pinned ${resourceType}.`);
  return pin;
}

function parseProviderProfile(value: unknown): ImageProviderProfilePayload {
  if (!isRecord(value)) throw new Wf09bCliError("WF09B_PROVIDER_PROFILE", "Image Provider Profile payload is invalid.");
  ...
  return {
    ...
    ...(capabilities === undefined ? {} : {
      capabilities: { acceptsReferenceMedia: capabilities.acceptsReferenceMedia === true }
    })
  };
}
...
function persistedPromptDecisions(qcDecision?: ImageQcDecision): SceneAssetDecisionPort {
  const unavailable = async (): Promise<never> => {
    throw new Wf09bCliError("WF09B_USAGE", "WF-09B does not redesign Scene Assets. Return to WF-09A for design changes.");
  };
  return {
    ...
    async compileImagePrompt(input) {
      ...
      if (!prompt) throw new Wf09bCliError("WF09B_ASSET_STATE", `Asset ${input.asset.id} has no materialized IMAGE_PROMPT.`);
      const negativePrompt = input.asset.design.negativePrompt?.trim();
      return { prompt, ...(negativePrompt ? { negativePrompt } : {}) };
    },
    async runImageQc() {
      if (qcDecision === undefined) throw new Wf09bCliError("WF09B_USAGE", "IMAGE_QC requires an explicit QC decision input.");
      return { ...qcDecision };
    }
  };
}
...
function validateQcDecision(value: unknown, label: string): ImageQcDecision {
  if (!isRecord(value)) throw new Wf09bCliError("WF09B_INPUT_INVALID", `${label} must be an object.`);
  const statuses = ["PASS", "PASS_WITH_NOTE", "FIXABLE", "REGENERATE", "REDESIGN", "REJECT"] as const;
  const severities = ["CRITICAL", "MAJOR", "MINOR"] as const;
  if (!statuses.includes(value.qcStatus as (typeof statuses)[number])) throw new Wf09bCliError("WF09B_INPUT_INVALID", `${label}.qcStatus is invalid.`);
  if (!severities.includes(value.severity as (typeof severities)[number])) throw new Wf09bCliError("WF09B_INPUT_INVALID", `${label}.severity is invalid.`);
...
export function parseQcPlan(value: unknown): Wf09bQcPlan {
  if (!isRecord(value) || !Array.isArray(value.results) || value.results.length === 0) {
    throw new Wf09bCliError("WF09B_INPUT_INVALID", "image-qc.json must contain a non-empty results array.");
  }
  const seen = new Set<string>();
  return {
    results: value.results.map((candidate, index): Wf09bQcInput => {
      ...
      if (seen.has(sceneId)) throw new Wf09bCliError("WF09B_INPUT_INVALID", `${label}.sceneId duplicates ${sceneId}.`);
      seen.add(sceneId);
      return {
        sceneId,
        ...(typeof candidate.mediaId === "string" ? { mediaId: candidate.mediaId.trim() } : {}),
...
async function importAdapter(moduleSpec: string): Promise<ImageProviderAdapter> {
  ...
  try {
    ...
    throw new Wf09bCliError("WF09B_ADAPTER_REQUIRED", `Could not load VPF_IMAGE_ADAPTER_MODULE (${trimmed}): ${error instanceof Error ? error.message : String(error)}`);
  }
  ...
  if (!isRecord(candidate) || typeof candidate.generate !== "function") {
    throw new Wf09bCliError("WF09B_ADAPTER_REQUIRED", "Image adapter must export default { generate(request) } or createImageProviderAdapter().");
  }
...
export class Wf09bCliService {
  ...
  private async resolveProviderProfile(status: ProjectStatus): Promise<ResolvedImageProviderProfile> {
    ...
    if (resource === null || resource.contentHash !== pin.contentHash) {
      throw new Wf09bCliError("WF09B_PROVIDER_PROFILE", "Pinned Image Provider Profile is missing or its content hash no longer matches.");
    }
    const payload = parseProviderProfile(resource.payload);
    if (payload.capabilities?.acceptsReferenceMedia !== true) {
      throw new Wf09bCliError("WF09B_PROVIDER_PROFILE", "WF-09 three-tier generation requires an Image Provider Profile with acceptsReferenceMedia=true.");
    }
    return { pin, payload };
  }
  ...
  private visualPorts(status: ProjectStatus) {
    ...
    return {
      bibles: new ChannelVisualBibleRegistryAdapter(this.registry, biblePin.resourceId, { [biblePin.version]: biblePin.contentHash }),
      formats: new FormatProfileRegistryAdapter(this.registry, formatPin.resourceId, { [formatPin.version]: formatPin.contentHash })
    };
  }

  private pipeline(repo: SqliteSceneAssetRepository, status: ProjectStatus, decisions: SceneAssetDecisionPort): SceneAssetPipeline {
    const ports = this.visualPorts(status);
    return new SceneAssetPipeline(repo, repo, ports.bibles, ports.formats, decisions, neverAutoApprove, systemClock, createSceneAssetIds());
  }

  private imageJobs(repo: SqliteSceneAssetRepository, status: ProjectStatus) {
    const ports = this.visualPorts(status);
    return createStandardReferenceAwareImageRuntimeJobService(
      repo,
      repo,
      ports.bibles,
      ports.formats,
  ...
  private primaryGenerateAssets(repo: SqliteSceneAssetRepository, projectId: string): Promise<ProductionAsset[]> {
    return repo.listAssets(projectId).then(assets => assets.filter(asset =>
      asset.assetClass === "PRIMARY_SCENE" && asset.owner.type === "SCENE" && asset.sourceStrategy === "GENERATE"
    ));
  }
  ...
  private assertRequiredSecrets(profile: ResolvedImageProviderProfile): RuntimeSecretRequirement[] {
    ...
    if (missing.length > 0) throw new Wf09bCliError("WF09B_SECRET_REQUIRED", `Missing required image runtime environment secret(s): ${missing.join(", ")}.`);
  ...
  async preflight(projectId: string) {
    ...
    try {
      ...
      const blocked = assets.filter(asset => asset.stale || asset.assetStatus !== "DESIGNED" || !asset.design.imagePrompt?.trim());
      if (assets.length === 0 || blocked.length > 0) throw new Wf09bCliError("WF09B_ASSET_STATE", `WF-09B requires current DESIGNED GENERATE assets; blocked=${blocked.map(item => item.id).join(",") || "none"}.`);
      return {
        projectId,
    ...
    try {
      ...
      const selected = selection === "ALL"
        ...
        : selection.map(assetId => {
            ...
            if (asset === undefined) throw new Wf09bCliError("WF09B_ASSET_STATE", `Unknown GENERATE PRIMARY_SCENE Asset: ${assetId}`);
            return asset;
          });
      if (selected.length === 0) throw new Wf09bCliError("WF09B_ASSET_STATE", "No DESIGNED image Assets are selected for execution.");
      ...
      registry.register({ provider: profile.payload.provider, jobType: "IMAGE_GENERATION", executor: new ImageRuntimeExecutor(adapter, { workspace: { workspaceRoot } }) });
      const orchestrator = new RuntimeOrchestrator(runtimeRepo, imageJobs.targetRevisionPort(), registry, systemClock, createRuntimeIds(), { workspaceRoot });
      const results: Array<Record<string, unknown>> = [];

      for (const asset of selected) {
        try {
          const scene = await sceneRepo.getScene(projectId, asset.owner.id);
          if (scene === null) throw new Wf09bCliError("WF09B_ASSET_STATE", `Scene is missing for Asset ${asset.id}.`);
          const knfBeat = classifyKnfBeat(scene);
          ...
          const outcome = await orchestrator.executeAutomated(projectId, prepared.job.id, { expectedOutputs: prepared.expectedOutputs, secretRequirements });
          const synchronized = await imageJobs.synchronizeRuntimeOutcome({ projectId, jobId: prepared.job.id });
          results.push({
            assetId: asset.id,
            sceneId: asset.owner.id,
            knfBeat,
            referenceCount: prepared.runtimeInput.references.length,
            referenceRoles: prepared.runtimeInput.references.map(reference => reference.role),
          ...
          results.push({ assetId: asset.id, sceneId: asset.owner.id, jobStatus: "SETUP_ERROR", resultStatus: "FAILED", error: error instanceof Error ? error.message : String(error) });
        }
      }
      return {
        projectId,
  ...
  async retryFailed(projectId: string, selection: "ALL" | string[]) {
    ...
    const runtimeRepo = new SqliteRuntimeExecutionRepository(status.projectDbPath);
    try {
      const rows = sceneRepo.db.prepare(`SELECT id FROM provider_jobs WHERE project_id = ? AND lifecycle_status = 'ACTIVE' AND job_type = 'IMAGE_GENERATION' AND status = 'FAILED' ORDER BY created_at`).all(projectId) as Array<{ id: string }>;
      const selectedIds = selection === "ALL" ? rows.map(row => row.id) : selection;
      if (selectedIds.length === 0) throw new Wf09bCliError("WF09B_ASSET_STATE", "No FAILED image Provider Jobs are available for retry.");
      const imageJobs = this.imageJobs(sceneRepo, status);
      const registry = new RuntimeExecutorRegistry();
      const workspaceRoot = path.dirname(path.dirname(status.projectRoot));
      registry.register({ provider: profile.payload.provider, jobType: "IMAGE_GENERATION", executor: new ImageRuntimeExecutor(adapter, { workspace: { workspaceRoot } }) });
      ...
      for (const jobId of selectedIds) {
        try {
          ...
          const outcome = await orchestrator.executeAutomated(projectId, retry.job.id, { expectedOutputs: retry.expectedOutputs, secretRequirements });
          const synchronized = await imageJobs.synchronizeRuntimeOutcome({ projectId, jobId: retry.job.id });
          results.push({ retryOfJobId: jobId, jobId: retry.job.id, assetId: synchronized.id, assetStatus: synchronized.assetStatus, resultStatus: outcome.result.status });
        } catch (error) {
          results.push({ retryOfJobId: jobId, resultStatus: "FAILED", error: error instanceof Error ? error.message : String(error) });
        }
      }
      return { projectId, requested: selectedIds.length, completed: results.filter(row => row.assetStatus === "CANDIDATE_AVAILABLE").length, failed: results.filter(row => row.assetStatus !== "CANDIDATE_AVAILABLE").length, results };
    } finally {
      runtimeRepo.close();
      sceneRepo.close();
    }
  ...
  async applyQc(projectId: string, file: string) {
    ...
    try {
      ...
      for (const item of plan.results) {
        ...
        if (asset === null) throw new Wf09bCliError("WF09B_ASSET_STATE", `Scene ${item.sceneId} has no PRIMARY_SCENE Asset.`);
        const mediaId = item.mediaId ?? (() => {
          if (asset.candidateMediaIds.length !== 1) throw new Wf09bCliError("WF09B_QC_AMBIGUOUS", `Scene ${item.sceneId} requires mediaId because it has ${asset.candidateMediaIds.length} candidates.`);
          return asset.candidateMediaIds[0]!;
        })();
        const result = await this.pipeline(repo, status, persistedPromptDecisions(item.decision)).runImageQc({ projectId, assetId: asset.id, mediaId, format: status.project.format });
        results.push({ sceneId: item.sceneId, assetId: asset.id, mediaId, qcId: result.qc.id, qcStatus: result.qc.qcStatus, assetStatus: result.asset.assetStatus });
      }
      return { projectId, qcCount: results.length, passCount: results.filter(item => item.qcStatus === "PASS" || item.qcStatus === "PASS_WITH_NOTE").length, results };
    } finally { repo.close(); }
  }
  ...
  async approve(projectId: string, selection: "ALL" | string[], approvedById?: string) {
    ...
    try {
      ...
      if (selected.length === 0) throw new Wf09bCliError("WF09B_ASSET_STATE", "No image Assets are ready for approval.");
      ...
      for (const asset of selected) {
        ...
        for (const mediaId of asset.candidateMediaIds) {
          ...
          if (qc?.qcStatus === "PASS" || qc?.qcStatus === "PASS_WITH_NOTE") approvable.push(mediaId);
        }
        if (approvable.length !== 1) throw new Wf09bCliError("WF09B_QC_AMBIGUOUS", `Asset ${asset.id} must have exactly one PASS/PASS_WITH_NOTE candidate; found ${approvable.length}.`);
        const result = await pipeline.approveAsset({ projectId, assetId: asset.id, mediaId: approvable[0]!, ...(approvedById === undefined ? {} : { approvedById }) });
        results.push({ assetId: asset.id, sceneId: asset.owner.id, mediaId: approvable[0], approvalId: result.approval.id, assetStatus: result.asset.assetStatus });
      }
  ...
  async status(projectId: string) {
    ...
    try {
      const assets = await this.primaryGenerateAssets(repo, projectId);
      const jobs = repo.db.prepare(`SELECT id, target_id AS assetId, status, attempt, error_code AS errorCode, result_media_ids_json AS resultMediaIdsJson FROM provider_jobs WHERE project_id = ? AND lifecycle_status = 'ACTIVE' AND job_type = 'IMAGE_GENERATION' ORDER BY created_at`).all(projectId) as Array<{ id: string; assetId: string; status: string; attempt: number; errorCode: string | null; resultMediaIdsJson: string }>;
      return {
        projectId,
        assetCount: assets.length,
        ...
        assets,
        jobs: jobs.map(job => ({ ...job, resultMediaIds: JSON.parse(job.resultMediaIdsJson) as string[] }))
      };
    } finally { repo.close(); }
  }
}
