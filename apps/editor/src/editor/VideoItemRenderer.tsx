...
export const VideoItemRenderer:FC<{item:VideoTimelineItem;masterVolume:number;muted?:boolean}>=({item,masterVolume,muted=false})=>(
  ...
  objectFit={item.fit}
  style={{width:"100%",height:"100%",opacity:item.opacity,transform:`translate(${item.x}px, ${item.y}px) scale(${item.scale}) rotate(${item.rotation}deg)`,transformOrigin:"center center"}} />
  </AbsoluteFill>
);
