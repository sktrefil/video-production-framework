...
const ItemRenderer:FC<{item:TimelineItem;masterVolume:number;trackMuted:boolean;project:EditProject}>=({item,masterVolume,trackMuted,project})=>{
  ...
  if(isAudioItem(item)){const duckingRanges=item.type==="BGM"?bgmDuckingRanges(project,item):[];return <AudioItemRenderer item={trackMuted?{...item,muted:true}:item} masterVolume={masterVolume} duckingRanges={duckingRanges}/>;}
