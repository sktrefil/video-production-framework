...
export const ImageItemRenderer: FC<{item: ImageTimelineItem}> = ({
  item,
}) => {
  ...
  const interpolateMotion = (
    from: number,
    to: number,
  ) =>
    interpolate(frame, [0, endFrame], [from, to], {
  ...
  const x = motion
    ? interpolateMotion(motion.from.x, motion.to.x)
    : item.x;
  const y = motion
    ? interpolateMotion(motion.from.y, motion.to.y)
    : item.y;
  const scale = motion
    ? interpolateMotion(motion.from.scale, motion.to.scale)
    : item.scale;
  const rotation = motion
    ? interpolateMotion(motion.from.rotation, motion.to.rotation)
    : item.rotation;
  const opacity = motion
    ? interpolateMotion(motion.from.opacity, motion.to.opacity)
    : item.opacity;
...
const ItemRenderer: FC<{
  ...
  if (item.type === "VIDEO") {
    return <VideoItemRenderer item={item} masterVolume={masterVolume} />;
  }
  if (item.type === "IMAGE") {
    return <ImageItemRenderer item={item} />;
  }
  if (isAudioItem(item)) {
    return <AudioItemRenderer item={item} masterVolume={masterVolume} />;
  }
...
export const ProjectRenderer: FC<{project: EditProject}> = ({project}) => {
  ...
  const renderableItems = project.items
    .map((item, index) => ({item, index, track: trackMap.get(item.trackId)}))
    .filter(
      (entry) =>
        entry.item.enabled && entry.track?.enabled === true,
    )
    .sort(
      (left, right) =>
        (left.track?.order ?? 0) - (right.track?.order ?? 0) ||
    ...
    <AbsoluteFill style={{backgroundColor: "black", overflow: "hidden"}}>
      {renderableItems.map(({item}) => (
        <Sequence
          ...
          name={`${item.trackId} · ${item.type} · ${item.id}`}
          layout="none"
        >
          <ItemRenderer
            item={item}
            masterVolume={project.settings.masterVolume}
          />
        </Sequence>
      ))}
    </AbsoluteFill>
