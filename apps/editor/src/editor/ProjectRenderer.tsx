...
const ItemRenderer:FC<{item:TimelineItem;masterVolume:number;trackMuted:boolean}>=({item,masterVolume,trackMuted})=>{
  ...
  if(isAudioItem(item))return <AudioItemRenderer item={trackMuted?{...item,muted:true}:item} masterVolume={masterVolume}/>;
