...
export const SubtitleGeneratorPanel:FC=()=>{
  ...
  const listStyle:CSSProperties={marginTop:6,height:listExpanded?"55vh":listHeight,minHeight:100,maxHeight:"55vh",resize:listExpanded?"none":"vertical",overflow:"auto",border:"1px solid #3f454d"};
  const rememberListHeight=(element:HTMLDivElement)=>{if(!listExpanded)setListHeight(Math.round(element.getBoundingClientRect().height));};
  const analyze=async()=>{
    setBusy(true);setError(null);setPreview(null);setApprovedIds(new Set());
  ...
  const apply=async()=>{
    ...
    try{
      const promoted=await promoteSubtitleAudioSync(project);
      dispatch(editorActions.loadProject(promoted.project));
  ...
  return <section data-editor-subtitle-panel style={{padding:8,fontSize:11}}>
    <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
      ...
      <button data-editor-command="toggle-subtitle-list-size" aria-pressed={listExpanded} onClick={()=>setListExpanded(value=>!value)}>{listExpanded?"Collapse subtitles":"Expand subtitles"}</button>
    ...
    {preview?<div data-editor-subtitle-sync-preview title="Drag the lower edge to resize this subtitle list" style={listStyle} onMouseUp={event=>rememberListHeight(event.currentTarget)}>
    ...
    {!preview?<div data-editor-subtitle-list title="Drag the lower edge to resize this subtitle list" style={listStyle} onMouseUp={event=>rememberListHeight(event.currentTarget)}>{items.map(item=><div key={item.id} style={{display:"grid",gridTemplateColumns:"110px 1fr",gap:6,padding:"3px 5px",borderBottom:"1px solid #30343a"}}><button onClick={()=>dispatch(editorActions.selectItem([item.id]))}>{item.id}</button><input value={item.text} onChange={event=>dispatch(editorActions.updateText(item.id,event.currentTarget.value))}/></div>)}</div>:null}
  </section>;
};
