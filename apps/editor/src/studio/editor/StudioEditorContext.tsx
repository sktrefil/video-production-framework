...
import {configuredStudioProjectConnection,hasEditorPersistenceAdapter,loadPersistedEditorProject,savePersistedEditorProject} from "./persistence/editorPersistenceApi";
...
const connectionInitialProject=(project:EditProject,projectId:string|undefined):EditProject=>projectId===undefined?project:{...project,project:{...project.project,id:projectId},tracks:[],items:[]};
export const StudioEditorProvider:FC<{children:ReactNode;initialProject?:EditProject}>=({children,initialProject})=>{
  ...
  const connection=configuredStudioProjectConnection();
  const connectedInitialProject=connectionInitialProject(initialProject??createEditorState().project,connection.projectId);
  const [state,dispatch]=useReducer(editorHistoryReducer,connectedInitialProject,createEditorState);
  const initialRef=useRef(connectedInitialProject);const latestRef=useRef(state.project);latestRef.current=state.project;
  ...
  const requestedProjectId=connection.projectId??initialRef.current.project.id;
  const adapterReady=isStudio&&!isReadOnlyStudio&&hasEditorPersistenceAdapter();
  useEffect(()=>{if(!adapterReady)return;let cancelled=false;void (async()=>{setPersistence((p)=>({...p,status:"loading",hydrated:false,error:null}));try{const loaded=await loadPersistedEditorProject(requestedProjectId);if(cancelled)return;if(loaded){if(loaded.schemaVersion!==1||loaded.project.id!==requestedProjectId)throw new Error(`Editor server returned an incompatible project: ${loaded.project.id}`);dispatch(editorActions.loadProject(loaded));}setPersistence((p)=>({...p,status:"idle",hydrated:true,error:null}));}catch(error){if(!cancelled)setPersistence((p)=>({...p,status:"error",hydrated:true,error:error instanceof Error?error.message:String(error)}));}})();return()=>{cancelled=true};},[adapterReady,requestedProjectId]);
  ...
  const reloadProject=useCallback(async()=>{if(!adapterReady)return false;const loaded=await loadPersistedEditorProject(requestedProjectId);if(!loaded)return false;if(loaded.schemaVersion!==1||loaded.project.id!==requestedProjectId)throw new Error(`Editor server returned an incompatible project: ${loaded.project.id}`);dispatch(editorActions.loadProject(loaded));return true;},[adapterReady,requestedProjectId]);
