...
export const loadStudioRenderProject=async(options?:{signal?:AbortSignal}):Promise<EditProject|null>=>{const response=await fetch("/vpf-active-editor-project.json",{cache:"no-store",signal:options?.signal});if(response.status===404)return null;if(!response.ok)throw new Error(`Studio render snapshot failed (${response.status})`);const project=await response.json() as EditProject;if(project?.schemaVersion!==1||!project.project||!Array.isArray(project.items)||!Array.isArray(project.tracks))throw new Error("Studio render snapshot is invalid");return project;};
