...
export type EditorProjectSettings = {snapEnabled:boolean; snapToleranceFrames:number; timelineZoom:number; masterVolume:number; clipAudioMasterVolume?:number; forcedEndFrame?:number};
...
export type VideoTimelineItem = BaseTimelineItem & MediaSourceWindow & {type:"VIDEO"; playbackRate:number; loop?:boolean; volume:number; x:number; y:number; scale:number; rotation:number; opacity:number; fit:"cover"|"contain"; transitionInFrames?:number; sourceUsagePolicy?:VideoSourceUsagePolicy; canonicalSourceStartFrame?:number; canonicalSourceDurationInFrames?:number; sourceWindowApprovalRequired?:boolean};
...
export type AudioDuckingSettings = {enabled:boolean; duckVolume:number; attackFrames:number; releaseFrames:number; minGapFrames:number};
export type AudioTimelineItem = BaseTimelineItem & MediaSourceWindow & {type:"TTS"|"CLIP_AUDIO"|"BGM"|"SFX"; volume:number; muted:boolean; fadeInFrames:number; fadeOutFrames:number; loop?:boolean; ducking?:AudioDuckingSettings};
...
export type SubtitleEmphasisRange = {start:number; end:number; color:string; enabled:boolean};
export type SubtitleTimelineItem = BaseTimelineItem & TextStyleFields & {type:"SUBTITLE"; generationSource?:SubtitleGenerationSource; generatedFromTtsIds?:string[]; emphasisRanges?:SubtitleEmphasisRange[]};
